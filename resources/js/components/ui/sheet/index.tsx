...
>(({ side = "left", className, children, ...props }, ref) => (
    <SheetPortal>
        ...
        >
            {children}
            <SheetPrimitive.Close className="absolute right-4 top-4 rounded-sm opacity-70 ring-offset-white transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-zinc-950 focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-zinc-100 text-gray-500">
                <X className="h-4 w-4" />
...
>(({ className, ...props }, ref) => (
    <SheetPrimitive.Title ref={ref} className={`text-lg font-semibold text-zinc-950 dark:text-zinc-50 ${className}`} {...props} />
));
...
>(({ className, ...props }, ref) => (
    <SheetPrimitive.Description ref={ref} className={`text-sm text-gray-500 dark:text-zinc-400 ${className}`} {...props} />
));
